...
static bool native_bridge2_initialize(const NativeBridgeRuntimeCallbacks *art_cbs,
                                      ...
                                      const char *isa)
{
    ALOGI("native_bridge2_initialize: enter isa=%s cache=%s",
          isa ? isa : "(null)", app_code_cache_dir ? app_code_cache_dir : "(null)");
    if (is_native_bridge_enabled()) {
        NativeBridgeCallbacks *cb = get_callbacks();
        ALOGI("native_bridge2_initialize: backend cb=%p version=%u initialize=%p",
              cb, cb ? cb->version : 0u,
              cb ? reinterpret_cast<void*>(cb->initialize) : nullptr);
        // Break the tail-call (so a crash keeps this frame) and guard against a
        // backend that exposes a NULL initialize pointer, which would otherwise
        // fault inside libnativebridge's InitializeNativeBridge with rip=0.
        if (cb && cb->initialize) {
            const bool r = cb->initialize(art_cbs, app_code_cache_dir, isa);
            ALOGI("native_bridge2_initialize: backend initialize returned %d", r);
            return r;
        }
        ALOGW("native_bridge2_initialize: backend unusable (cb=%p, initialize=%p)",
              cb, cb ? reinterpret_cast<void*>(cb->initialize) : nullptr);
    } else {
...
// v7: replaces getTrampoline starting with interface version 7.
static void *native_bridge7_getTrampolineWithJNICallType(void *handle, const char *name,
                                                         const char *shorty, uint32_t len,
                                                         enum JNICallType jni_call_type)
{
    ALOGV("enter native_bridge7_getTrampolineWithJNICallType %s", name);
    NativeBridgeCallbacks *cb = get_callbacks();
    if (cb && cb->version >= 7 && cb->getTrampolineWithJNICallType) {
        return cb->getTrampolineWithJNICallType(handle, name, shorty, len, jni_call_type);
    }
    // Fallback to the v2 trampoline for older backends.
    return cb ? cb->getTrampoline(handle, name, shorty, len) : nullptr;
}

// v7: trampoline for a raw function pointer (used by RegisterNatives).
static void *native_bridge7_getTrampolineForFunctionPointer(const void *method, const char *shorty,
                                                            uint32_t len, enum JNICallType jni_call_type)
{
    ALOGV("enter native_bridge7_getTrampolineForFunctionPointer");
    NativeBridgeCallbacks *cb = get_callbacks();
    if (cb && cb->version >= 7 && cb->getTrampolineForFunctionPointer) {
        return cb->getTrampolineForFunctionPointer(method, shorty, len, jni_call_type);
    }
    return nullptr;
}

// v8: identify whether a function pointer belongs to the native bridge address
// space. A16 ART calls this for every RegisterNatives entry (see b/393035780),
// so it MUST be implemented or ART dereferences a null callback and crashes.
static bool native_bridge8_isNativeBridgeFunctionPointer(const void *method)
{
    ALOGV("enter native_bridge8_isNativeBridgeFunctionPointer");
    NativeBridgeCallbacks *cb = get_callbacks();
    if (cb && cb->version >= 8 && cb->isNativeBridgeFunctionPointer) {
        return cb->isNativeBridgeFunctionPointer(method);
    }
    return false;
}
